...
impl Path {
    ...
    pub fn parse(parser: &mut Parser) -> Result<Self, ParserError> {
        ...
        Ok(Path { 
            ...
            full_path: Fragment { source: Arc::clone(&head.fragment.source), range: matched_source_range }, 
            head,
            tail 
        })
...
fn parse_segment(parser: &mut Parser) -> Option<Identifier> {
    ...
    const VALID_SEGMENT_SEQUENCES: [&[TokenTy]; 4] = [
        &[TokenTy::Whitespace, TokenTy::ColonColon, TokenTy::Whitespace, TokenTy::Identifier],
        &[TokenTy::Whitespace, TokenTy::ColonColon, TokenTy::Identifier],
        &[TokenTy::ColonColon, TokenTy::Whitespace, TokenTy::Identifier],
...
mod test_path {
    use crate::{
        ast::path::Path, lexer::Lexer, parser::Parser, source_tracking::{filename::FileName, source::Source, SourceMap}
    };
