    ...
    where Self: Sized 
    {
        ...
        for (prefix, radix) in prefixes {
            if let Some(prefix_stripped) = literal.strip_prefix(prefix) {
                ...
                // report it. 
                let value: BigUint = BigUint::from_str_radix(fully_stripped, radix)
                    .map_err(|err: ParseBigIntError| IntegerLiteralParsingError::NumParsingError {
                        error: err,
                        at: fragment,
                    })?;
        ...
        let value: BigUint = BigUint::from_str_radix(literal, 10)
            .map_err(|err| IntegerLiteralParsingError::NumParsingError { error: err, at: fragment })?;
