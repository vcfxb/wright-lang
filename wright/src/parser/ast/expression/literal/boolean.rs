...
pub fn parse_boolean_literal<'src>(parser_state: &mut ParserState<'src>) -> NodeParserResult<BooleanLiteral<'src>> {
    ...
    for (token_ty, value) in [(TokenTy::True, true), (TokenTy::False, false)] {
        ...
        if let Some(IndexedToken { index, token: Token { length, .. } }) = parser_state.next_token_if_ty_eq(token_ty) {
            // On success, return the popped token's appropriate AST node. 
            return Ok(BooleanLiteral { meta: parser_state.make_ast_node_meta(index, length), value });
        }
    }
