...
use super::{NodeParserOption, NodeParserResult, BoxedParserFn};

/// Return a [Box]xed parser function that returns an [`Option`] rather than a [`Result`]. 
pub fn discard_errors<'src, PF, O>(parser_function: PF) -> BoxedParserFn<'src, NodeParserOption<'src, O>>
where PF: (Fn(ParserState<'src>) -> NodeParserResult<O>) + 'static
{
    Box::new(move |parser_state: ParserState<'_>| {
        ((parser_function)(parser_state)).ok()
    })
}
