...
use super::{NodeParserResult, ParserSuccess, BoxedParserFn};
...
where 
    ...
    MF: (Fn(O1) -> O2) + 'static,
{
    Box::new(move |parser_state: ParserState| {
        (map_function)((parser_function)(parser_state))
    })
}

/// Map specifically the node
pub fn map_node_type<'src, PF, MF, N1, N2>(parser_function: PF, map_function: MF) -> BoxedParserFn<'src, NodeParserResult<'src, N2>> 
where
    ...
    MF: (Fn(N1) -> N2) + 'static
{
    Box::new(move |parse_state: ParserState<'_>| {
        ...
        parse_result.map(|ParserSuccess { updated_parser_state, ast_node }| {
            ParserSuccess { updated_parser_state, ast_node: ((map_function)(ast_node)) }
        })
    })
}
