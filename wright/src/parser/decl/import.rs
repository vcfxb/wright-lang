...
use crate::{ast::{decl::import::ImportDecl, path::Path}, lexer::token::{Token, TokenTy}, parser::{error::{ParserError, ParserErrorKind}, whitespace, Parser}, source_tracking::fragment::Fragment};
...
impl ImportDecl {
    ...
    pub fn parse(parser: &mut Parser) -> Result<Self, ParserError> {
        let use_kw: Token = parser
            .next_if_is(TokenTy::KwUse)
            .ok_or(ParserErrorKind::ExpectedImportDeclaration.at(parser.peek_fragment_or_rest_cloned()))?;
        ...
        if let Some(semi) = parser.next_if_is(TokenTy::Semi) {
            ...
            Err(ParserErrorKind::ImportMustEndWithSemicolon.at(parser.peek_fragment_or_rest_cloned()))
        }
    }
}
