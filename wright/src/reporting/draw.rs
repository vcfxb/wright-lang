...
mod tests {
    ...
    fn test_diagnostic(d: &Diagnostic, write_unicode: bool) -> String {
        ...
        // Write to buffer.
        super::draw(d, &mut buffer, write_unicode).expect("Wrote diagnostic to buffer");
    ...
    fn test_unicode_vs_ascii() {
        use super::{
            ASCII_SECTION_CLOSER, ASCII_SECTION_OPENER, ASCII_SECTION_PREFIX,
            UNICODE_SECTION_CLOSER, UNICODE_SECTION_OPENER, UNICODE_SECTION_PREFIX,
        };

        // Print unicode section:
        println!(
            "{UNICODE_SECTION_OPENER} Note: \
