...
impl<W: WriteColor> Renderer<W> {
    ...
    fn draw_code_section(&mut self, diagnostic: &Diagnostic) -> io::Result<()> {
        ...
        let primary_highlight: &Highlight = diagnostic
            .primary_highlight
            .as_ref()
        ...
        let primary_location =
            match (primary_highlight.fragment.source.name(), self.supports_hyperlinks) {
        ...
        let vertical_right_branch = self
            .style
            .vertical_right_char()
            .unwrap_or(self.style.vertical_char());
        ...
        writeln!(
            &mut self.writer,
            "{vertical_right_branch}{a}{horizontal_down_branch}{b}",
            // Add two for spaces.
            ...
            b = horizontal_dashed
                .to_string()
                .repeat(self.width.unwrap_or(60) as usize - (line_num_width + 4))
        )?;
        ...
        let mut highlights_by_source: HashMap<u64, Vec<&Highlight>> =
            HashMap::with_capacity(diagnostic.secondary_highlights.len() + 1);
        ...
        for line_indice in min_line_index..max_line_index {
            writeln!(
                &mut self.writer,
                "{vertical} {line_num:>line_num_width$} {vertical}",
                line_num = line_indice + 1
            )?;
        }
    ...
    fn draw_note(&mut self, note: &OwnedString) -> io::Result<()> {
        ...
        color_spec.set_italic(true).set_underline(true);
...
mod tests {
    ...
    use crate::{
        reporting::{style::Style, Diagnostic, Highlight},
        source_tracking::{
            filename::FileName, fragment::Fragment, source::Source, source_ref::SourceRef,
        },
    };
    use indoc::indoc;
    ...
    fn print_with_highlight_and_note() -> io::Result<()> {
        let source = Source::new_from_static_str(
            FileName::Test("test.wr"),
            indoc! {"
            func main() {
        ...
        let frag = Fragment {
            source: SourceRef(Arc::new(source)),
            range: 0..12,
        };
