...
fn bitwise_xor(input: Fragment) -> IResult<Fragment, Expression> {
    parser_left(base_primary, parse_xor)(input)
}

/// Parse a 'bitwise and' binary expression.
fn bitwise_and(input: Fragment) -> IResult<Fragment, Expression> {
    parser_left(base_primary, parse_and)(input)
}
