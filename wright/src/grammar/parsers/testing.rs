...
impl TestingContext {
    ...
    pub fn test_all_fail<'a, T>(
        &'a self,
        parser: fn(Fragment<'a>) -> IResult<Fragment<'a>, T>
    ) {
        assert!(
            self.run_parser_on_all(parser)
                .iter()
                .enumerate()
                .map(|(ind, r)| {
                    if !r.is_err() {
                        let source = self.get_fragment(ind).source();
                        println!("Parser succeeded on \"{}\" when it should have failed. (test case {})",
                            source,
                            ind
                        );
                    }
        ...
        assert!(
            self.run_parser_on_all(parser)
                ...
                .map(|(ind, r)| {
                    if !r.is_ok() {
                        let source = self.get_fragment(ind).source();
                        println!("Parser failed on \"{}\". (test case {})",
                                 source,
                                 ind
                        );
                    }
